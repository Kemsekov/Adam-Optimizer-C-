...
/// </summary>
public class FisherInformation
{
    /// <summary>
    /// Epsilon that is used to compute gradients
    /// </summary>
    public double Epsilon{get;set;} = (double)1e-7;
    /// <summary>
    /// Function that is used to generate some output from input samples
    /// </summary>
    public Func<double[], double> ModelFunction { get; }
    /// <summary>
    ...
    /// </param>
    public FisherInformation(Func<double[], double> fun, Func<double[],double[]>? gradient = null)
    {
        ModelFunction = fun;
    ...
    /// to your model.
    /// </returns>
    public virtual double[]? OverSample(double[] sample)
    {
        var sampleClone = sample.ToArray();
        var dims = sample.Length;
        var gradient = Gradient(sample);
        if (gradient.Any(double.IsNaN)) return null;
    ...
    public double[] OverSamples(IEnumerable<double[]> samples)
    {
    ...
    /// </returns>
    public double[] OverGenerated(Func<double[]> generateSample, int expectationsSampleCount)
    {
    ...
    /// </summary>
    public static double CentralDifference(Func<double[], double> func, double[] input, int paramIndex, double epsilon)
    {
        input[paramIndex] += epsilon;
        var left = func(input);
        input[paramIndex] -= 2 * epsilon;
        var right = func(input);
        return (left - right) * 0.5 / epsilon;
    }
    /// <summary>
    /// Approximates gradient of a function using central difference
    /// </summary>
    public static double[] GradientCentralDifference(Func<double[], double> func, double[] sample,double epsilon)
    {
        var sampleClone = sample.ToArray();
        var dims = sample.Length;
        var gradient =
